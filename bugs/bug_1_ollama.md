# Evaluation Case 01 (OLLAMA): [Short Bug Description]
- **Target Repository:** rich
- **Evaluation Date:** 2026-06-07 23:13:53
...
================== 1 failed, 955 passed, 25 skipped in 4.11s ===================
```
...
The test `test_decode_newlines` is failing because the `Text.from_ansi()` method is not correctly handling Windows-style newlines (`\r\n`) when converting to plain text. The assertion `assert Text.from_ansi("Hello\r\nWorld\r\n").plain == "Hello\nWorld\n"` is failing because the actual output of `Text.from_ansi().plain` is `\n\n`, which is different from the expected output `"Hello\nWorld\n"`. This suggests that the method is incorrectly replacing Windows-style newlines with a single Unix newline character.

### 3. Proposed Code Fix
To fix this issue, you need to modify the `Text.from_ansi()` method to correctly handle Windows-style newlines when converting to plain text. You can do this by adding a check for `\r\n` and replacing it with a single Unix newline character (`\n`) only if necessary.

Here's an example of how you could modify the `Text.from_ansi()` method:
```python
class Text:
    # ...
    ...
    def from_ansi(cls, ansi):
        # ...
        plain = ansi.replace("\r\n", "\n")  # Add this line to replace Windows-style newlines with Unix newline
        return cls(plain)
```
Alternatively, you could also modify the `Text.from_ansi()` method to preserve the original Windows-style newlines in the plain text output.

### 4. Priority Level
High

This issue is considered high-priority because it affects the correctness of the `Text.from_ansi()` method, which is a critical component of the Rich library. The failure of this test indicates that the library may produce incorrect results when handling Windows-style newlines in ANSI escape sequences.
