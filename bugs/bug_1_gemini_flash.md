# Evaluation Case 01 (GEMINI_FLASH): [Short Bug Description]
- **Target Repository:** rich
- **Evaluation Date:** 2026-06-07 23:14:09
...
================== 1 failed, 955 passed, 25 skipped in 4.15s ===================
```

## 2. LLM Proposed Code Fix (Output)
### 1. Failing Test(s)
* `../rich/tests/test_ansi.py::test_decode_newlines`

### 2. Root Cause Analysis
The failure occurs because the ANSI decoder processes carriage return characters (`\r`) literally. In a terminal context, `\r` resets the cursor to the beginning of the line. When decoding `"Hello\r\nWorld\r\n"`, the decoder interprets the `\r` as an instruction to overwrite the preceding text on that line, resulting in the text `"Hello"` and `"World"` being cleared, leaving only the newlines (`'\n\n'`).

### 3. Proposed Code Fix
To fix this, normalize Windows-style line endings (`\r\n`) to standard newlines (`\n`) before decoding the ANSI string. Update the `decode` method in `rich/ansi.py` (or `Text.from_ansi` in `rich/text.py`) to replace `\r\n` with `\n`:

```python
# In rich/ansi.py (inside AnsiDecoder.decode) or rich/text.py (inside Text.from_ansi)
